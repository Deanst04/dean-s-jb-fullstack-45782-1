...
(() => {
  ...
  console.log(`medium`);
  const usersStates = users
  .map(({ address: { state } }) => `${state}`)
  .reduce((acc, currState) => {
    if (!acc.includes(currState)) acc.push(currState);
        return acc
  }, [])
  ...
  console.log(`hard`);
  const eyeColorsDistribution = users.reduce((acc, { eyeColor }) => {
    const color = eyeColor;
    if (acc[color]) acc[color] += 1
    else acc[color] = 1
    return acc
  }, {})
  console.log(eyeColorsDistribution);
  console.log(`<=================================>`);

  console.log(`extreme`);
    const usersWeightDistribution = users.reduce((acc, { weight , address : { coordinates:  { lat } } }) => {
    if (lat > 0) {
        acc.north = acc.north || {totalWeight: 0, totalUsers: 0}
        acc.north.totalWeight += weight
        acc.north.totalUsers +=1
    }
    else if (lat < 0) {
        acc.south = acc.south || {totalWeight: 0, totalUsers: 0}
        acc.south.totalWeight += weight
        acc.south.totalUsers +=1
    }
    return acc
  }, {})
  console.log(usersWeightDistribution);
  console.log(`The average weight of the northen users is: ${(usersWeightDistribution.north.totalWeight / usersWeightDistribution.north.totalUsers).toFixed(2)}`);
  console.log(`The average weight of the southern users is: ${(usersWeightDistribution.south.totalWeight / usersWeightDistribution.south.totalUsers).toFixed(2)}`);
  console.log(`<=================================>`);
